...
namespace SnapperCodingChallenge.ConsoleApplication
{
    ...
    public static class Services
    {
        ...
        public static IOptions Settings { get; private set; }

        public static void InitialiseSnapperImage(SnapperImageType type)
        {
            if (type == SnapperImageType.TextFile) 
            {
            ...
            else
            {
                ...
                throw new Exception("Switch statement invalid - contact developer...!");
            }
        }

        public static void InitialiseTargetImages(TargetImageType type)
        {
            if (type == TargetImageType.TextFile)
            {
                //Get array of absolute filepaths for SnapperImages (although there should only be 1!)
                ...
                List<TargetImageTextFile> targetImages = new List<TargetImageTextFile>();
        ...
        public static void InitialiseLogger(LoggerType type)
        {
            ...
            else if (type == LoggerType.TextFile)
            {
                Logger = new LoggerTextFile(snapperConsoleLogFilePath);
            }
            else
            {
                //TODO change to switch statement.
        ...
        public static void InitialiseOutput(LoggerType type)
        {
            if (type == LoggerType.Console)
            {
                Logger = new LoggerConsole();
        ...
        {
            if (type == OptionsType.TextFile)
            {
                Settings = new OptionsTextFile(optionsFilePath);
            }
            else
            {
                //TODO change to switch statement.
